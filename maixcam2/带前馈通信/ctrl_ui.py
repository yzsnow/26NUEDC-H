...
class CtrlUI:
    RUN_BUTTONS = ("HOLD", "Q6", "TASK3", "STOP", "ZERO", "LIGHT", "TUNE")
    ...
    BTN_TEXT_SCALE = 1.8
    TUNE_BTN_TEXT_SCALE = 1.0
    BAR_TEXT_SCALE = 1.4
    TUNE_BAR_TEXT_SCALE = 1.0
    TUNE_VALUE_TEXT_SCALE = 1.35
    TASK_PROMPT_TEXT_SCALE = 3.0
    # The clock is for the operator only, so keep it quiet and out of the main
    # camera area instead of presenting it like a judging result.
    TIME_TEXT_SCALE = 1.15
    ...
    def draw_run(self, img, ctx):
        ...
        mode, sub, fps, ball_valid, task_time_s, task_prompt,
        ...
        # Keep the task clock small, white and in the upper-right corner.  It is
        # an operator reference, not a public pass/fail indicator.
        time_h = 0
        t_s = ctx.get("task_time_s")
        ts = self.TIME_TEXT_SCALE
        if t_s is not None:
            txt = "{:.2f}s".format(t_s)
            size = image.string_size(txt, scale=ts, thickness=2)
            img.draw_string(self.w - size.width() - 6, self.top_h + 2,
                            txt, self.c_txt, scale=ts, thickness=2)
            time_h = size.height()
        elif ctx.get("task_prompt"):
            # Before the run, keep the existing large prompt: it tells the
            # operator whether TASK3 is arming or waiting for the second press.
            txt = ctx["task_prompt"]
            ready = ctx.get("task_prompt_ready")
            prompt_scale = self.TASK_PROMPT_TEXT_SCALE
            size = image.string_size(txt, scale=prompt_scale, thickness=3)
            img.draw_string(6, self.top_h + 2, txt,
                            self.c_ok if ready else self.c_target,
                            scale=prompt_scale, thickness=3)

        # The two scored errors stay on the left, clear of the private clock.
        if ctx.get("task_result"):
            img.draw_string(6, self.top_h + 14,
        ...
        if ctx.get("ff_active") or abs(ctx.get("ff_theta_deg", 0.0)) > 0.01:
            cascade += " F{:+.1f}".format(ctx.get("ff_theta_deg", 0.0))
        size = image.string_size(cascade, scale=1.2, thickness=2)
        cascade_y = self.top_h + 8
        if time_h:
            cascade_y = self.top_h + time_h + 6
        img.draw_string(self.w - size.width() - 6, cascade_y, cascade,
                        self.c_txt, scale=1.2, thickness=2)

        self._draw_buttons(img, self.run_rects)

    def draw_tune(self, img, now_ms):
        s = self.TUNE_BAR_TEXT_SCALE
        img.draw_rect(0, 0, self.w, self.top_h, self.c_bar, -1)
        label, path, step, low, high = self.params[self.sel]
        value = get_param_value(self.cfg, path)
        ...
        font = self.tune_font
        body_scale = self.TUNE_VALUE_TEXT_SCALE
        size = _text_size(body, body_scale, 2, font)
        while body_scale > 0.7 and size.width() > self.w - 16:
            body_scale = round(body_scale - 0.1, 2)
            size = _text_size(body, body_scale, 2, font)
        ...
        _draw_text(img, bx, by, body, self.c_dark, body_scale, 2, font)
