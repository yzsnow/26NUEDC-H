...
| 赛题项目 | 本仓库对应实现 | 当前说明 |
| --- | --- | --- |
| 1. 图传显示与录像 | **不属于本仓库**；由外部 DJI Action 5 Pro 独立完成 | Action 5 Pro 自带供电、存储/回放和接收链路，与本仓库所有 MCU、MaixCAM2、传感器和电机控制线路无电气连接 |
...
赛题第 1 问的图传、录像和回放由外部 DJI Action 5 Pro 系统负责，不计入本仓库的代码功能；本仓库只提供钢球识别与摆杆控制所需的本地相机画面。
...
flowchart LR
    ...
    Z --> B[凹槽摆杆与钢球]
    D[外部 DJI Action 5 Pro] -.独立安装/供电.-> R[外部接收、显示与存储]
```
...
赛题第 1 问不在上述控制闭环内。DJI Action 5 Pro 负责车载图像采集、无线图传、外部显示、
录像和回放，使用自身电池与存储/接收设备；它不接入 MSPM0G3507、MaixCAM2、G356、TB6612、
ZDT 或 OLED 的任何电源线、地线、UART、PWM 和 GPIO。MaixCAM2 屏幕上的识别叠加画面只服务
于钢球检测和现场调试，不能替代赛题要求的独立图传系统。
...
| DJI Action 5 Pro（外部图传设备） | 1，独立完成赛题第 1 问的车载拍摄、图传、录像和回放；不属于本仓库控制系统 |
...
### 赛题第 1 问：DJI Action 5 Pro（仓库外）

- Action 5 Pro 固定在车体上方，镜头覆盖整根凹槽摆杆；按 DJI 设备说明使用自身电池、
  存储卡和无线接收/显示设备。
- Action 5 Pro 不与 MSPM0G3507、MaixCAM2、G356、TB6612、ZDT 或 OLED 共用电源、地线和
  信号线；本仓库没有 DJI 控制、图传、录像、文件回放或接收端程序。
- 赛题现场应单独确认画面覆盖完整、实时显示稳定、录像文件可回放。MaixVision 的开发调试
  预览和 MaixCAM2 本地显示不计作赛题第 1 问的图传链路。
...
1. 在 MaixVision 中打开 `maixcam2/带前馈通信/`。MaixVision 仅用于部署和本地调试预览，
   不承担赛题第 1 问的图传、录像或回放。
...
`maixcam2/自动采集训练集/main.py` 是与比赛控制程序分离的图片采集工具。它提供 `640x480`
本地预览，可选择或新建 `/root/data/image/project_数字/` 训练项目；按下开始后等待 3 秒，之后
每隔 3 秒保存一张 JPEG。图片按 `img_连续编号_light_on/off.jpg` 命名，切换补光后等待曝光稳定，
继续已有项目时从最大序号加一，因此不会覆盖原图；保存失败会自动停止采集。

该工具不加载 YOLO 模型、不连接 ZDT 电机或底盘，也不录像、不向外传输画面。它只用于赛前
制作钢球检测训练数据，不属于赛题第 1 问的图传设备或软件。
...
- 赛题第 1 问完全由外部 DJI Action 5 Pro 图传系统承担；当前仓库不提供图传发送、接收、
  录像、存储或回放功能。MaixCAM2 的相机画面只作为钢球识别输入和本地调试画面。
